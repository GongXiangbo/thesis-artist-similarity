...
│   ├── model1_latent_space_metadata.ipynb # TripletNet1 metadata/t-SNE 分析
...
`model1.ipynb` 的 ROC-AUC/AP 为 `nan`，notebook 输出说明当时运行环境缺少 `sklearn`，因此阈值分析仍有 precision/recall/F1，但 ROC-AUC 和 average precision 被跳过。当前 `code/requirements.txt` 已包含 `scikit-learn`；重新运行该 notebook 时应可补齐这两个指标。
...
## Latent-space metadata 分析

`code/model1_latent_space_metadata.ipynb` 是最新新增的 TripletNet1 专用分析 notebook。它用于在 `model1.ipynb` 训练完成后，只重跑 latent-space 与 metadata 一致性分析：
...
- 输出保存到 `code/checkpoints/TripletNet1/analysis/latent_metadata/`，包括 projection PNG/CSV、`TripletNet1_latent_with_metadata.csv`、group similarity summary 和 silhouette summary。
...
只重跑 TripletNet1 metadata/t-SNE 分析时，在 Jupyter 中打开并运行：

```text
code/model1_latent_space_metadata.ipynb
```

该 notebook 期望已有 TripletNet1 latent CSV 或 best checkpoint；两者都在 `code/checkpoints/` 下，因体积/运行产物原因不会提交到 git。
...
`model2.ipynb` 到 `model5.ipynb` 的已保存输出仍显示 metadata-aware t-SNE 被跳过，因为这些 notebook 的旧 cell 只在 notebook 工作目录附近搜索 metadata。最新的 `model1_latent_space_metadata.ipynb` 已改为从项目根目录的 `data/metadata/` 读取，并保存了 TripletNet1 的 metadata/t-SNE 结果。若后续要对 TripletNet2~5 做同样分析，应复用这个 notebook 的 metadata 路径逻辑。
