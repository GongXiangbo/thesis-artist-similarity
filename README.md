...
`model1.ipynb` 的 ROC-AUC/AP 为 `nan`，notebook 输出说明当时运行环境缺少 `sklearn`，因此阈值分析仍有 precision/recall/F1，但 ROC-AUC 和 average precision 被跳过。
...
notebook 输出显示 metadata-aware t-SNE、country/genre group similarity 被跳过，原因是运行时没有在 notebook 工作目录找到 metadata CSV。仓库的 metadata 位于 `data/metadata/`；如果需要恢复这部分分析，应更新 notebook 中的 metadata 搜索路径。
