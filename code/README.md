...
上面两个训练命令显式传入的是当前四个 notebook 都选出的最佳 margin `0.1`。notebook 会完整 sweep `[0.1, 0.3, 0.5, 0.7, 0.9]`，并按 validation retrieval MRR 选择最佳 margin。
...
四个主训练 notebook 会从 `../data/metadata/` 查找 metadata；当前保存输出使用 `artists_genre_country.csv`，并补充 `artists.csv` 中的 artist name。
...
所有主训练 notebook 的图像都会保存到 `figures/<model>/`，例如 `model1.ipynb` 保存到 `figures/model1/`。PCA/t-SNE 图使用同一批 sampled artists；t-SNE 会先 PCA 预降维到最多 50 维，再用 cosine t-SNE 生成一套 2D 坐标，country/genre 图只更换着色标签，不重复拟合不同坐标。

### `model1.ipynb` 到 `model4.ipynb`

四个主训练 notebook 当前统一为 fixed CSV negatives，并已全部跑完：
...
四个模型的最佳 margin 都是 `0.10`，每个模型的 OOF validation triplets 都是 `1132`。当前 notebook 输出摘要如下：

| 模型 | aggregation | mean validation MRR | OOF ranking acc | OOF margin acc | ROC-AUC | Average precision | retrieval MRR | retrieval P@1 |
| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| `TripletNet1` | `stack` | `0.311444` | `90.46%` | `87.90%` | `0.9254` | `0.9127` | `0.405530` | `0.275850` |
| `TripletNet2` | `mean` | `0.278678` | `91.61%` | `84.45%` | `0.9127` | `0.9003` | `0.326258` | `0.202147` |
| `TripletNet3` | `mean` | `0.274923` | `92.05%` | `85.78%` | `0.9113` | `0.8895` | `0.262694` | `0.150626` |
| `TripletNet4` | `mean` | `0.270821` | `90.46%` | `84.63%` | `0.9129` | `0.8998` | `0.333466` | `0.209302` |

`TripletNet1` 是当前综合最强模型：它在 mean validation MRR、OOF ROC-AUC/AP、retrieval MRR 和 retrieval P@1 上都领先。`TripletNet3` 的 OOF ranking accuracy 最高，但 full-corpus retrieval 明显落后。
...
这轮 fixed-triplet 结果中，三个 legacy baseline 的最佳 margin 也都是 `0.10`。`TripletNet2` 和 `TripletNet4` 的 retrieval MRR 接近但均低于 `TripletNet1`；`TripletNet3` 虽然 OOF ranking accuracy 最高，retrieval MRR/P@1 却是四者最低。
...
四个 notebook 中的跨模型比较 cell 会尝试读取 checkpoint summary CSV。由于 checkpoint 目录不在仓库中，跨模型汇总可能来自本机运行缓存。做论文表格时建议优先使用每个 notebook 自己的 margin summary、OOF triplet summary 和 retrieval summary；本 README 当前记录的是四个主 notebook 各自最新输出中的结果。

`model1.ipynb` 到 `model4.ipynb` 的 metadata cell 会依次从 `../data/metadata/`、`data/metadata/` 和当前目录查找 metadata，因此从 `code/` 目录或项目根目录启动 Jupyter 时都能读取 `artists_genre_country.csv`。
